...
RBC Pro는 Bullet Physics 기반 강체 시뮬레이션으로 물리 법칙 자체는 존재하지만, 타이어 슬립 모델 및 마찰 상세 스펙이 공개 자료상 미확인인 시각적 재현에 최적화된 환경. 반면, Genesis는 Full-system Newton constraint solver 로 타이어 동역학, 방향성 마찰, 서스펜션 등을 포함한 제어·로보틱스용 정밀 물리 시뮬레이터.

**RBC Pro (Blender):**
* Bullet Physics 기반 강체 시뮬레이션 / Sequential Impulse 
* 제약 조건을 순차적으로 근사 처리
* 수치 정확도 최적화 (속도 > 정확도)
* 타이어 슬립/슬립각 모델 구현 미확인 (공개 공식 자료상)
* 마찰 모델 상세 스펙 미확인 (공개 공식 자료상)
* 영상·애니메이션 시각적 재현 목적
...
**Genesis:**
* Full-system Newton constraint solver
* 모든 제약 조건을 연립방정식으로 묶어 동시 처리 
* 수치 정확도 최적화 (정확도 > 속도)
* 타이어-지면 마찰, 슬립각 모델링
* 방향성 마찰 콘
* 제어·로보틱스 정확도 목적
