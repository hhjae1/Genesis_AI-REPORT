...
* **Equation (Loss Function)**:
  다음 수식을 최소화하는 최적 입력 $(T^*, S^*)$를 탐색합니다.
  
  $$(T^*, S^*) = \text{argmin}_{T,S} \left( |a^G(T,S) - a^B| + |\kappa^G(T,S) - \kappa^B| \right)$$
