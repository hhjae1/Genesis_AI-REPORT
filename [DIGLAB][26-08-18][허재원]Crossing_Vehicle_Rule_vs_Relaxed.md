...
- ③(저속)은 감속량이 둘 다 −6.0 km/h 로 같음. 이 장면의 차이는 감속 크기가 아니라 **교차점 도달 시점**임(아래 참고)
...
| | 충돌 | 규칙 개입 | 제동 최대 | 기준 대비 감속 | 지연 |
|---|---|---|---|---|---|
| 규칙 | 없음 | 0~8 s (교차점 부근에서는 꺼짐) | 0.29 | −6.0 km/h | **+9.0 m** |
| 완화 학습 | **있음** | — | 0.42 | −6.0 km/h | **+14.1 m** |

- **감속량은 둘이 같음**(−6.0 km/h). 완화 학습이 제동을 오히려 더 밟았음(0.42 vs 0.29). 이 장면의 차이는 감속의 크기가 아님
- 차이는 **시점**임. 완화 학습이 규칙보다 **5 m 더 지연**되어(+14.1 vs +9.0 m) 교차점 도달 시각이 진입차와 겹침. 충돌 순간 진입차가 **앞 2.6 m·옆 2.3 m** 로 바로 앞에 있었음
- 규칙은 이 장면에서 크게 감속하지 않고 통과함. 규칙 개입은 초반(자차가 아직 멀고 느릴 때)에만 걸렸고 교차점 부근에서는 꺼져 있었음 — 결과적으로 **더 일찍 빠져나간 것**이 갈랐음
