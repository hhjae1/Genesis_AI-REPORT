...
- 🚨 **충돌 직전 조향이 튐.** 완화 학습의 조향이 8.5 s 에 +0.18 이었다가 0.3 s 만에 −0.40 으로 **부호가 뒤집힘**. 조향 변화율이 평시 0.14/s 에서 **6.29/s 로 45배**
...
- 조향이 튄 순간부터 **제동을 놓음**(0.35 → 0.11 → 0.00). 감속을 포기하고 조향으로 갈아탐
- 횡 이탈이 −0.71 m 로 한쪽에 갔다가 **+0.67 m 로 반대쪽으로 넘어가며** 진입차와 겹침
- 3.2 절에서 본 **조향 회피 습관이 이 셀에서는 충돌을 만든 것**임. 다른 셀에서 통했던 회피 방식이 여기서는 진입차 쪽으로 향했음
- 제동 시점 자체는 늦지 않았음. 8.1 s(진입차 8 m)부터 밟기 시작해 8.7 s 에 0.35 까지 올렸으나, 조향으로 갈아타며 **그 제동을 놓아** 교차점 앞에서 속도를 못 줄임
