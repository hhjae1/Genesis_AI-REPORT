...
- [미해결] 진입 차량 양보 (가로지르거나 끼어드는 차, **그림 2 참조**) — 9회 시도, 충돌 9~10/10 으로 대조군과 동일 🔴

**이번 결과 / 막힌 것 / 다음**
- 결과: 정지 장애물 6개 경로 6/6 통과, 최소 여유 0.64 m
- 막힌 것: 진입 차량 양보 — 종방향 권한이 PID 에 있어 RL 이 감속을 실행 못 함 **판단 필요** 🔴
...
**그림 2.** 시뮬레이터 화면. 진입 차량은 **자차 경로를 가로지르거나 경로로 끼어드는 차**를 말함. 정지물처럼 서 있는 게 아니라 자차 도달 시각에 맞춰 교차 지점(검은 링)을 지나감. 각 차량이 출발 위치에 서 있고, 표시가 교차점 쪽으로 갈수록 굵어지는 것이 진행 방향.
...
| 좌회전 합류 | +20° | +0.25 | 도로로 꺾어 들어옴 |
| 우회전 합류 | −20° | −0.25 | 반대쪽에서 꺾어 들어옴 |
...
한 줄 요약: **원인은 보상이 아니라 속도 제어 권한. 정책이 감속을 내도 실행이 안 됨.**
...
-> 양보는 기준속도 **아래로** 내려가는 행동이라 PID 와 정면으로 싸워야 함. 정지 장애물은 조향 한 번으로 끝나 속도 권한이 필요 없었고, 그래서 안 드러났음

**대응**: 제동 명령이 제동력만이 아니라 **PID 목표속도도 함께 낮추도록** 변경. 가속 페달을 떼고 제동을 거는 것과 같음. 재학습 결과 확인 중
