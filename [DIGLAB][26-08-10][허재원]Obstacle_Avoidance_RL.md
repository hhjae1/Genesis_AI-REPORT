...
**대조군** = 같은 정책·같은 체크포인트에 **장애물 관측만 0 으로 가린 것**. 주행 실력과 물리 조건은 그대로고 장애물 정보만 없어지므로, 둘의 차이가 곧 정보를 쓴 능력. 잔차 0(BC+PID)을 대조군으로 쓰면 학습에 안 쓴 경로에서 추종 오차가 10.9 m 라 장애물 근처를 지나지도 않음.  
**회피율 x/40** = 시험 경로 10개(`shp_*` 5개 + `hsnew*` 5개) × 장애물 배치 지점 4곳 = 40셀 중 피한 셀 수. 대조군이 부딪힌 셀만 셈 — 대조군도 안 부딪히는 셀은 시험이 성립하지 않음.  
...
**충돌 x/10** = 시험 경로 10개(`shp_*` 5개 + `hsnew*` 5개) 중 진입 차량과 부딪힌 경로 수. 지터·가시거리 조건마다 따로 셈.
