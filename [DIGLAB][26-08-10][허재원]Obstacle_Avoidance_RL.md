...
| 번호 | 내용 |
|---|---|
| ① | 게이트를 **고정 거리**(표면거리 6 m)로 엶 — 속도와 무관 |
| ② | 게이트를 **`d_LPS`** 로 엶 — 속도에 따라 변함 (7 m/s 면 13 m, 25 m/s 면 46 m) |
...
- ① → ② 가 핵심. 90 km/h 에서 6 m 는 0.24 초라 이미 늦음  
-> 회피 기동은 20~50 m 전에 시작해야 하는데 그 구간에서는 경로 이탈이 처벌됨. 기준을 `d_LPS` 로 바꾸니 57.5% -> 97.4%
