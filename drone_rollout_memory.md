...
> PPO 구현에서는 rollout 끝나면 학습 후 discard → 항상 **“1 rollout 메모리”만 유지**
...
# 수정사항 
## 1. Drone_Epoch (학습 반복 단위)

PPO 같은 on-policy 알고리즘은 rollout(trajectory) 하나를 모은 뒤,  
그 데이터를 가지고 여러 번 학습을 돌림.
...
→ 하나의 rollout 데이터를 **5번 반복해서 학습**  

즉, **epoch = rollout 데이터셋을 몇 번 전체 학습하는가**  

---

## 2. Drone_Mini-batch (데이터 쪼개기)

rollout 전체를 한 번에 학습하면 너무 크고 비효율적  

그래서 rollout을 **여러 개의 작은 묶음(mini-batch)** 으로 쪼개서 학습  
...
→ rollout 하나를 **4등분해서 학습**


## 3. 정책 파라미터 공유

- **네트워크 파라미터 (38,277개 ≈ 153 KB)**  
  → 공유(shared) 됨  
  → 즉, 8192개 환경이 하나씩 따로 네트워크를 들고 있는 게 아님  

- PPO/강화학습에서는 **하나의 정책 네트워크**가 있고,  
  이 정책이 8192개 환경을 동시에 관찰하고 행동을 결정함  
...
### 메모리 효율
- 만약 환경마다 네트워크를 따로 둔다면:  
  `153 KB × 8192 ≈ 1.2 GB`  
- 쓸데없이 낭비됨  

### 강화학습 구조
- 목표는 **“모든 환경에서 얻은 경험을 하나의 정책이 학습”** 하는 것  
- 따라서 정책 네트워크는 하나만 있으면 충분  

---

## 5.  그럼 8192개 환경에서 필요한 건 뭐냐?

- 네트워크는 공유하지만,  
  환경별 **상태(state), 보상(reward), done flag, action** 같은 **rollout 데이터**는 환경별로 따로 저장됨  
- 이게 바로 앞에서 계산했던 **약 75 MB / rollout**에 해당하는 부분  
