...
![Custom 4-ray ground sensing pattern](images_jw/ray_pattern_concept.png)

→ 각 wheel 중심에서 -z 방향으로 ray 1 개씩 → 총 4 개 ray. `hit_distance` 를 받아 `compression = REST_D − hit_distance` 로 변환, 이후 비대칭 댐퍼 식으로 N 계산.
**Per-wheel 독립 감지**라서 heightfield 의 ridge/dip 에 wheel 별로 다른 compression 이 적용됨 (그림: 전륜은 ridge 위 → 짧은 hit_distance / 후륜은 dip 안 → 긴 hit_distance).
...
![비대칭 댐퍼 개념도](images_jw/asymmetric_damper_concept.png)

→ 압축 (ċ > 0) 에서는 큰 댐퍼 force 가 새시를 위로 밀어 (C_COMP=14000) 자세를 안정화하고,
신장 (ċ < 0) 에서는 작은 댐퍼 force 만 새시를 아래로 잡아당겨 (C_EXT=4000) wheel 이 ground 를 빠르게 추적할 수 있게 함. 화살표 **방향과 크기 모두 비대칭**.
...
#### 왜 racing-style 비대칭 (C_COMP > C_EXT) 인가

승용차는 일반적으로 **C_COMP < C_EXT** (압축 부드러움, 신장 단단함) — 충격을 천천히 흡수해 운전자 jolt 를 최소화하는 comfort 우선 설정. 반면 본 모델은 **C_COMP > C_EXT** (압축 단단함, 신장 부드러움) — 레이싱카 / 시뮬레이터 표준.

선택 이유:

| 우선순위 | 이유 |
|---------|------|
| **1. 업계 표준** | CARLA, Unity Vehicle Physics, Unreal Chaos Vehicles 모두 동일 방식. 자율주행/시뮬 표준. |
| **2. 핸들링 예측성** | 압축 단단 → 새시 자세 변화 (dive, squat, roll) 최소 → AI/제어기가 예측·제어 쉬움. |
| **3. 거동 통일** | Blender 학습 데이터가 CARLA 기반 physics 로 생성됨 → 동일 댐퍼 거동으로 sim-to-real gap 최소화. |
| **4. 수치 안정성** | 진동 빨리 감쇠 → solver 발산 ↓, 큰 DT 가능. (부수 효과) |

#### 작은 댐퍼로 시 발생하는 수치 안정성 문제

만약 comfort 스타일 (C_COMP < C_EXT, 또는 양쪽 모두 작게) 로 설정하면 wheel 이 격렬히 출렁이며 ray-wheel 계산에 다음 문제들이 생김:

| 문제 | 영향 |
|------|------|
| **압축 속도 ↑** | `comp_rate = (compression - prev) / DT` 가 매 frame 갑자기 커짐. `COMP_RATE_CLAMP = 30 m/s` 안전 상한에 자주 걸림. |
| **N 진동** | suspension normal force 가 매 frame 크게 흔들림 → 마찰력 (μ·N) 도 진동 → tire force 불안정. |
| **Ray 거리 진동** | hit distance 변화 빨라짐 → `compression` 계산 noisy → 다음 frame N 계산에 noise 누적. |
| **omega 진동** | wheel rotation 속도가 출렁 따라 흔들림 → slip ratio κ 진동 → F_long noisy. |
| **DT ↓ 강제** | 위 문제들 안정화 위해 더 작은 timestep 필요 → 시뮬 계산량 ↑. |

→ Ray 계산 자체의 알고리즘 복잡도는 댐퍼와 **무관** (매 frame 4 ray, O(1)). 다만 **시뮬 수치 안정성** 측면에서 큰 댐퍼가 유리하며, 이는 큰 DT 와 큰 batch (n_envs=200 의 MPPI imagination rollout) 를 안정적으로 운영할 수 있게 해줌.

``→ 즉, C_COMP > C_EXT 선택의 본질은 "업계 표준 + Blender 데이터 매칭" 이고, 수치 안정성은 부수 효과로 따라옴.``

---

## 5. Coulomb vs Pacejka 비교
...
-> 기존 mesh-mesh contact 방식에서 계산 방식을 Pacejka로 할 수 있지만, ray가 계산 속도 측면에서 우월하고(충돌 탐색이 간단) 자동차 sim에서 표준 방식이라 채택(자세한 내용은 아래 첨부)
